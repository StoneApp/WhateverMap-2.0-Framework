...
"boundingBox":[-20037508.34278924,-20037508.34278924,20037508.34278924,20037508.34278924]
		...
		,
		{
			...
			"address":["http://wms.cuzk.cz/wms.asp?REQUEST=GetMap&SERVICE=wms&VERSION=1.1.1&SRS=EPSG:4258&WIDTH=256&HEIGHT=256&FORMAT=image/png&BBOX=%@,%@,%@,%@&LAYERS=KN&TRANSPARENT=TRUE", 
				"{tileLU.x}", 
				"{tileRB.y}", 
				"{tileRB.x}", 
				"{tileLU.y}"],
			"addressSRS":"+proj=longlat +ellps=GRS80 +no_defs ",
			...
			"projectionKey44":"+proj=latlong +ellps=WGS84 +towgs84=0,0,0 +nodefs",
			"boundingBox44":[-180, -85.05112878, 180, 85.05112878],			
			"georeferencingTyp44e":0,	
			"coordToPixelX1":"(16777216 + 5340354 * dtor({x})) / (2 ^ zoom)",
			"coordToPixelY1":"(16777216 - 5340354 * log((1 + sin(dtor({y}))) / (1 - sin(dtor({y})))) / 2) / (2^zoom)",
			"coordToPixelX44":"({x}+180)/360",
			"coordToPixelY44":"0.5-ln((1+sin({y}*pi/180))/(1-sin({y}*pi/180)))/(4*pi)",
			"pixelToCoordX44":"({x}/256)/(2^{zoom})*360.0-180.0",
			"pixelToCoordY44":"(arctan(sinh(pi*(1-2*({y}/256)/(2^{zoom})))))*180.0/pi",
